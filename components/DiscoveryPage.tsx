...
const DiscoveryPage: React.FC<DiscoveryPageProps> = ({ onSelectApp, onSelectCreator, onNavigateLeaderboard }) => {
  ...
  const filteredApps = useMemo(() => {
    let result = [...apps];
    if (result.length === 0) result = [...MOCK_APPS]; // Double safety
              ...
              <div className="flex flex-col items-center text-center">
                <div className="relative mb-4 md:mb-6">
                  <div className="w-16 h-16 md:w-20 md:h-20 rounded-full border-4 border-gray-50 p-1 bg-white relative z-10 overflow-hidden shadow-sm">
                    <img src="https://picsum.photos/seed/elena/100" className="w-full h-full rounded-full object-cover" alt="Elena Voss" />
                  </div>
                  <div className="absolute -bottom-1 -right-1 scale-75 z-20">
                    <Badge type="consistent_shipper" showTooltip={false} />
                  </div>
                </div>
                <h5 className="text-base md:text-lg font-bold text-gray-900 mb-1">Elena Voss</h5>
                <p className="text-[10px] md:text-xs font-bold text-gray-300 uppercase tracking-widest mb-4 md:mb-6">@evoss</p>
                <p className="text-[11px] md:text-xs text-gray-400 font-medium italic mb-6 md:mb-8 leading-relaxed max-w-[200px]">"Minimalist architect shaping soulful digital rituals."</p>

                <div className="grid grid-cols-3 gap-4 md:gap-6 w-full mb-8 md:mb-10 pb-8 md:pb-10 border-b border-gray-50">
                  <div className="flex flex-col">
                    <span className="text-sm md:text-base font-black text-gray-900">4</span>
                    <span className="text-[8px] font-black text-gray-300 uppercase tracking-widest">Jams</span>
                  </div>
                  <div className="flex flex-col">
                    <span className="text-sm md:text-base font-black text-gray-900">1.1k</span>
                    <span className="text-[8px] font-black text-gray-300 uppercase tracking-widest">Upvotes</span>
                  </div>
                  <div className="flex flex-col">
                    <span className="text-sm md:text-base font-black text-gray-900">2.4k</span>
                    <span className="text-[8px] font-black text-gray-300 uppercase tracking-widest">Fans</span>
                  </div>
                </div>

                <button className="w-full py-4 rounded-2xl bg-gray-900 text-white text-[10px] md:text-[11px] font-black uppercase tracking-[0.2em] shadow-xl shadow-gray-900/10 hover:bg-gray-800 transition-all active:scale-95">
                  Follow
                </button>
              </div>
            </div>
