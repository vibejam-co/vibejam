...
interface Comment {
  id: string;
  user: { name: string; avatar: string; handle: string; badge?: string; };
  text: string;
  timestamp: string;
...
const AppView: React.FC<AppViewProps> = ({ project, onClose, onCreatorClick, isLoggedIn = false, onAuthTrigger, onManageJam, isOwner = false }) => {
  ...
  const [comments, setComments] = useState<Comment[]>([
    { id: '1', user: { name: 'Jordan T.', handle: '@jt', avatar: 'https://picsum.photos/seed/jordan/100', badge: 'top_curator' }, text: "The attention to detail on the shadow occlusion here is incredible. Is this using a custom shader?", timestamp: '2h ago', parentId: null },
    { id: '1-1', isCreator: true, user: { name: project.creator.name, handle: project.creator.handle, avatar: project.creator.avatar, badge: project.creator.badges?.[0]?.type }, text: "Thanks Jordan! Optimized for low-power devices.", timestamp: '1h ago', parentId: '1' },
    { id: '2', user: { name: 'Maya R.', handle: '@maya', avatar: 'https://picsum.photos/seed/maya/100' }, text: "Cleanest UI I've seen all week. Absolute vibe.", timestamp: '5h ago', parentId: null }
  ]);
  ...
  const handlePostComment = () => {
    ...
    const comment: Comment = { id: Date.now().toString(), user: { name: 'Curator X', handle: '@curatorx', avatar: 'https://picsum.photos/seed/user-curator/100' }, text: newComment, timestamp: 'Just now', parentId: null };
  ...
  const handlePostReply = (parentId: string) => {
    ...
    const reply: Comment = { id: Date.now().toString(), user: { name: 'Curator X', handle: '@curatorx', avatar: 'https://picsum.photos/seed/user-curator/100' }, text: replyText, timestamp: 'Just now', parentId };
    ...
    return (
      <div key={c.id} className={`flex gap-3 ${isReply ? 'ml-8 mt-6' : 'mt-10 first:mt-0'}`}>
        <div className="relative shrink-0 aura-clip">
          ...
          {c.isCreator && <div className="aura-halo" style={{ background: auraColor, opacity: 0.15, inset: '-2px' }} />}
        </div>
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1">
            <span className="text-sm font-bold text-gray-900">{c.user.name}</span>
            {c.isCreator && <span className="px-1.5 py-0.5 rounded bg-gray-900 text-[8px] font-black text-white uppercase tracking-wider">Creator</span>}
          ...
          <div className="flex items-center gap-4">
            {!isReply && <button onClick={() => setReplyingTo(replyingTo === c.id ? null : c.id)} className="text-[10px] font-black text-gray-300 hover:text-gray-900 uppercase tracking-widest transition-colors">Reply</button>}
          </div>
          {replyingTo === c.id && (
            <div className="mt-4 flex flex-col gap-2">
              <textarea autoFocus value={replyText} onChange={(e) => setReplyText(e.target.value)} placeholder={isLoggedIn ? `Reply to ${c.user.name}...` : 'Sign in to reply'} className="w-full bg-gray-50 border border-gray-100 rounded-xl p-3 text-sm focus:bg-white focus:border-blue-200 transition-all outline-none min-h-[80px] resize-none" />
              <div className="flex justify-end gap-2">
                ...
                <button onClick={() => handlePostReply(c.id)} disabled={!isLoggedIn || !replyText.trim()} className="px-4 py-1.5 rounded-lg bg-gray-900 text-white text-[10px] font-black uppercase tracking-widest disabled:opacity-20">Post</button>
              </div>
            </div>
          )}
      ...
      <div className="relative w-full max-w-5xl bg-white rounded-[40px] shadow-2xl border border-gray-100 overflow-hidden animate-in slide-in-from-bottom-8 duration-500">
        <div className="sticky top-0 z-30 flex items-center justify-between px-8 h-20 bg-white/90 backdrop-blur-md border-b border-gray-50/50">
          <button onClick={onClose} className="group flex items-center gap-2 text-gray-400 hover:text-gray-900">
            <div className="w-8 h-8 rounded-full border border-gray-100 flex items-center justify-center text-gray-400"><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M15 19l-7-7 7-7"/></svg></div>
            <span className="text-[11px] font-black uppercase tracking-[0.2em]">Close View</span>
          </button>
          <div className="flex items-center gap-3">
            {isOwner && <button onClick={onManageJam} className="px-5 py-2 rounded-xl bg-gray-900 text-white text-[10px] font-black uppercase tracking-widest mr-4">Manage Jam</button>}
            <button onClick={() => toggleBookmark(project)} className={`w-10 h-10 rounded-full border flex items-center justify-center transition-all ${bookmarked ? 'bg-blue-50 border-blue-200 text-blue-500' : 'bg-white border-gray-100 text-gray-400 hover:text-gray-900'}`}><svg className="w-5 h-5" fill={bookmarked ? 'currentColor' : 'none'} stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M5 5a2 2 0 012-2h10a2 2 0 012 2v16l-7-3.5L5 21V5z"/></svg></button>
          </div>
        </div>
        <div className="flex flex-col md:flex-row">
          <div className="flex-1 p-8 md:p-12 lg:p-16 border-r border-gray-50">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-16">
              {[{ label: 'MRR', value: project.stats.revenue, accent: 'text-green-600' }, { label: 'Upvotes', value: project.stats.upvotes, accent: 'text-blue-500' }, { label: 'Growth', value: project.stats.growth, accent: 'text-purple-600' }, { label: 'Days Live', value: project.stats.daysLive, accent: 'text-gray-900' }].map((stat) => (
            ...
            <section className="pt-16 border-t border-gray-50">
              ...
              <div className="flex gap-4 mb-12">
                ...
                <div className="flex-1 space-y-3">
                  <textarea value={newComment} onChange={(e) => setNewComment(e.target.value)} placeholder={isLoggedIn ? "Write a signal..." : "Sign in to join the discussion"} onClick={() => !isLoggedIn && onAuthTrigger?.()} readOnly={!isLoggedIn} className="w-full bg-gray-50 border border-gray-100 rounded-2xl p-4 text-sm focus:bg-white focus:border-blue-200 transition-all outline-none min-h-[100px] resize-none" />
                  <div className="flex justify-end"><button onClick={handlePostComment} className="px-6 py-2 rounded-xl bg-gray-900 text-white text-[10px] font-black uppercase tracking-widest transition-all">{isLoggedIn ? "Post Signal" : "Sign In"}</button></div>
                </div>
              </div>
              <div className="space-y-4">{rootComments.map(c => renderComment(c))}</div>
            </section>
          </div>
          <div className="w-full md:w-[320px] lg:w-[380px] p-8 lg:p-12 shrink-0 bg-[#F9F9FB]/30">
            <div className="sticky top-28">
              <div onClick={() => onCreatorClick?.(project.creator)} className="bg-white p-8 rounded-[40px] border border-gray-100 shadow-sm hover:border-blue-100 transition-all cursor-pointer text-center">
                ...
                <button onClick={(e) => { e.stopPropagation(); if (!isLoggedIn) onAuthTrigger?.(); }} className="w-full py-4 rounded-2xl bg-gray-900 text-white text-[11px] font-black uppercase tracking-[0.2em] shadow-lg shadow-gray-900/10 active:scale-95 transition-all">Follow</button>
              </div>
            </div>
          </div>
        </div>
