      ...
      <div className="max-w-5xl mx-auto px-6">
        ...
        <header className="flex flex-col items-center text-center mb-16">
          ...
          <div className="flex items-center justify-center gap-2 mb-10 bg-white border border-gray-50 p-2 rounded-[32px] shadow-sm">
            <MetricItem label="Followers" value={user.stats.followers.toString()} onClick={() => {}} />
            <div className="w-px h-6 bg-gray-100" />
            <MetricItem label="Following" value={user.stats.following.toString()} onClick={() => {}} />
            <div className="w-px h-6 bg-gray-100" />
            <MetricItem 
              label="Bookmarks" 
          ...
          <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-20">
              {[
                ...
                { label: 'Upvotes', value: user.stats.upvotes.toString() },
                { label: 'Comments', value: user.stats.comments.toString() },
                { label: 'Followed', value: user.stats.followed.toString() }
              ].map(stat => (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                {bookmarks.map((bookmark) => {
                  const fullProject = MOCK_APPS.find(a => a.id === bookmark.id) || {
                    ...
                    stats: { revenue: bookmark.mrr, isRevenuePublic: true, growth: '0%', rank: 0, upvotes: 0, daysLive: 0, views: 0, bookmarks: 0 },
                    creator: { ...bookmark.creator, color: '#3b82f6', type: 'Solo Founder', badges: [] },
                    stack: [],
                    vibeTools: [],
                    milestones: []
                  } as AppProject;
