...
CREATE POLICY "Users can manage their own upvotes" ON public.jam_upvotes FOR ALL USING (auth.uid() = user_id);
CREATE POLICY "Upvotes are viewable by everyone" ON public.jam_upvotes FOR SELECT USING (true);
